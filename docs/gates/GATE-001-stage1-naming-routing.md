...
- `67975166b86c72772087db4c6b7bb78a20b9d639`：建立 13 项注册表并修正路由；
- `8ec132a8eee9d5917e6bcea29a5f8741224b0080`：强化 Schema、条目数量和 A/B 路由检查；
- `777cc42343bc18303773ffe1fc3fe5f51b981cb1`：收口历史名称并强化调用目标与条目结构检查。

验收证据：

- 注册表检查器退出码为 0，报告 13 个条目与 13 个实际 Skill；
- Gate 指定的历史调用检索无命中；
- 全量历史名称检索无命中；
- Luna 的全部变更文件均在授权范围内；
- 禁止文件、版本号、校验器和 v2 契约未被 Luna 修改；
- 新增说明、检查器输出与报错满足中文要求；
- Terra 第二次独立只读复审结果为 `PASS`；
- Sol 复跑验证通过，工作树在实施提交后保持干净。

阶段 1 已关闭。后续阶段必须使用新的独立 Gate，不得沿用本 Gate 扩大实施范围。
