...
读 `.movie-create/storyboard.json` 的 `shots[]`、`.movie-create/style-guide.md`（如有）、每镜对应的 `01-角色提示词/` 与 `02-场景提示词/` 冻结身份/空间事实，按镜头顺序渲染 `03-分镜脚本图提示词.md`。每次明确重新生成都从当前 storyboard 全量重建，不读取旧 03 做增量保留：逐镜按 `characters` 数组原顺序、再按 `scene`，首次出现即加入映射；角色精确解析 `01-角色提示词/{名}.md`，场景精确解析 `02-场景提示词/{名}.md`，零候选、规范化多候选、角色/场景同名冲突均失败，不模糊猜测。映射集合、类型和顺序必须与 storyboard 完全一致，额外孤儿或错序均失败；改名视为旧名删除、新名新增。默认采用**文档级一次性手动参考图槽位**：在“统一生成规格”下建立一次人读映射，后文各镜只写角色名、场景名并继承该映射。方括号仅为图片槽位，不得填入 Markdown/蓝图/ASCII/提示词文本。最终 03 只输出标题、段落名、生成规格、严格映射和可直接粘贴的图像提示词；内部路径、状态、操作说明不得复制到交付物。03 默认不展示账本状态，不生成 `<Picture N>`/`<图片N>`；只有用户明确要求自动平台适配时，才由 OUT 单独依据已验证资产和本次绑定编译平台标签。1镜=1×1、2镜=1×2、3–4镜=2×2、5–6镜=2×3、7–9镜=3×3，超过9镜分页，每页最多9格；每格必须映射现有 `shot_id`，不得新增剧情事实。旧 `03-分镜提示词.md` 仅只读兼容，禁止自动迁移、覆盖或删除。
...
- 统一生成规格先写一次“参考资产映射”，格式为 `角色名 = [角色名设定图]`、`场景名 = [场景名场景图]`；后文只写对应语义名称，不在每格重复方括号。仅为 01/02 已建立且需跨镜一致的角色、场景建立槽位。方括号仅作内部槽位语义，最终 03 映射块只能渲染标题和严格映射行；手动附图、未附图、图片存在性与操作解释均不得渲染。
- 03 不展示 `reference-assets.json`、`video-config.json`、`prompt_only`、`verified`、`bound` 等内部状态，不写模型专用图片字段或 `<Picture N>`、`<图片N>`。`ref_anchors` 仍保留在 JSON 作为机器语义字段，但不直接生成图片标签；自动平台标签只能由 OUT 在自动适配模式下编译。
...
- [ ] 统一生成规格只有一次人读参考资产映射；每格继承映射并只写实际出现的角色/场景名称
- [ ] 03 未泄漏账本状态、内部配置或模型专用图片标签；方括号槽位未冒充 Markdown/蓝图/提示词文本
...
| 触发 | 处理 |
|------|------|
| 素材信息不足 | 生成前询问或在内部选择确定值；LOCKED 后只写入确定提示词，不标注可替换 |
