...
读 `.movie-create/storyboard.json` 的 `shots[]`、`.movie-create/style-guide.md`（如有）、每镜对应的 `01-角色提示词/` 与 `02-场景提示词/` 冻结身份/空间事实，按镜头顺序渲染 `03-分镜脚本图提示词.md`。默认采用**文档级一次性手动参考图槽位**：在“统一生成规格”下建立一次人读映射，后文各镜只写角色名、场景名并继承该映射；方括号是用户待附的真实图片槽位，不是 Markdown/蓝图，也不代表图片已经存在。03 默认不展示账本状态，不生成 `<Picture N>`/`<图片N>`；只有用户明确要求自动平台适配时，才由 OUT 单独依据已验证资产和本次绑定编译平台标签。1镜=1×1、2镜=1×2、3–4镜=2×2、5–6镜=2×3、7–9镜=3×3，超过9镜分页，每页最多9格；每格必须映射现有 `shot_id`，不得新增剧情事实。旧 `03-分镜提示词.md` 仅只读兼容，禁止自动迁移、覆盖或删除。
...
- 统一生成规格先写一次“参考资产映射”，格式为 `角色名 = [角色名设定图]`、`场景名 = [场景名场景图]`；后文只写对应语义名称，不在每格重复方括号。仅为 01/02 已建立且需跨镜一致的角色、场景建立槽位，关键道具须有独立资产卡才可加入。方括号不得填入 `.md`、空间蓝图、ASCII 或提示词文本，未附图时不得声称已引用。
