...
🎯 **交付物归位四大块**（用户真正要的）：
- **① 角色块**：01-角色提示词/{角色名}.md × N（角色主卡：定妆+多视图）；表演参考图、穿戴物细节图仅按用户明确请求单独输出到同一目录
...
完成门槛：01 每个 Part、02 基础场景与变化线、03 每个实际宫格页、04 每个 S 段（仅在文件存在且 `target_model` 已锁定时）均必须通过 `../shared/scripts/validate_prompt_payload.cjs` 对应校验；任一失败不得标记完成或交付。
| 触发条件 | 一线修复 | 仍失败兜底 |
|---------|---------|-----------|
...
执行档位默认是标准。快速档位只有在时长、角色、场景、模型、比例与无未决绑定条件全部满足，且用户明确选择后才生效；快速仍保持四块交付，角色主卡始终为 Part1→Part2，复杂规划、审阅与 Gate 不因快速而隐式绕过。进入快速前必须一次性合并确认执行档位、模型与比例。
...
### 最终可见交付边界（直接模式）
直接模式写入磁盘的 01-角色提示词、02-场景提示词、03-分镜脚本图提示词、04-视频提示词，以及最终响应，都只允许标题、Part/段落名、画幅/分辨率、严格参考资产映射和可直接粘贴的创作提示词。禁止出现运行/流程/用户操作/免责/待办/内部状态。控制面（执行档位、Gate、状态、路径、校验和计划）只能留在交互消息或 `.movie-create/` 内部态，绝不能进入四块正文。主链暂停由 entry 统一拥有。
